...
public class UseFileStream {
    ...
    private void testFile() throws IOException {
        ...
        //  프로젝트와 무관하게 파일을 찾을
        File readMe = new File("C:\\Users\\samsung\\eclipse-workspace\\java_ssafy_8th_l\\src\\com\\ssafy\\live5\\io\\node\\readme.txt");
        System.out.println(readMe.exists());
        
        //  프로젝트 내부에서 파일을 찾을
        readMe = new File(".\\src\\com\\ssafy\\live5\\io\\node\\readme.txt");
        System.out.println(readMe.exists());
        readMe = new File(".\\bin\\com\\ssafy\\live5\\io\\node\\readme.txt"); //이게 좋은 방식 src는 나중에 어떻게 될지 모름
        System.out.println(readMe.exists());
        //  특정 클래스 기준으로 파일을 찾을
    ...
    public long fileMove(int bufferSize) {
        ...
        File src = new File("c:\\ssafy\\eclipse-jee-2018-09-win32-x86_64.zip");  //기존 파일 위치
        File target = new File("c:\\Temp\\eclipse.zip");  //옮길 위치
        // TODO: bufferSize 크기의 byte []을 이용해서 src를 target에 복사하시오.
        try (FileInputStream fin = new FileInputStream(src);
	        FileOutputStream fout = new FileOutputStream(target);){
	        int read = 0;
	        byte[] buffer = new byte[bufferSize];  //bufferSize 100
