...
public class UseJson {
    ...
    public List<BoxOffice> getBoxOffice() throws JsonParseException, JsonMappingException, IOException {
        ...
        Map<String, Map<String, Object>> result = mapper.readValue(json, Map.class); //Map형태로 json을 읽어서 줌
        List< Map<String,Object> > list = (List) result.get("boxOfficeResult").get("dailyBoxOfficeList");  //boxOfficeResult의 dailyBoxOfficeList 리스트를 받음
        for(Map<String, Object> info : list) {
        	BoxOffice boxOffice = mapper.convertValue(info, BoxOffice.class); //받은 info를 BoxOffice로 형변환
